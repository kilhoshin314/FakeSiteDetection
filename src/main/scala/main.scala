...
object Main {
  ...
  def readConfig(configFile: String) {
	...
	//println("support_vecs.value")
	//support_vecs.foreach{ value => println(value)}
	...
	def predict(target: String, convertMap: HashMap[String, Int]): Double = {

	  var targetStr = ""
	  try {
	    val htmlSource: BufferedSource = Source.fromURL(target, "utf-8")
		val htmlStr = htmlSource.mkString
      ...
      val invalidLabel = -1
      val dummyNode = new TreeNode(invalidLabel, ArrayBuffer[TreeNode]())
      val targetTree: TreeNode = IntTreeParser.parse(targetStr).getOrElse(dummyNode)
	  println("targetTree: " + targetTree)

	  //val test_vec = empty_node +: sc.parallelize((1 to samples.size)).map{index =>
      val test_vec = empty_node +: (1 to samples.size).par.map{index =>
		//println("index: " + index)
        val node = new libsvm.svm_node
        node.index = index
		//println("contains: " + support_vecs.contains(index))
        if(support_vecs.contains(index)) {
          val ptk = new PTK(alpha)
          //val targetTree = getTestTree(target)

		  val trainTree: TreeNode = getTrainTree(index)
		  //println("trainTree: " + trainTree)
          val temp = ptk.eval(targetTree, trainTree)
          //println("temp: " + temp)
          //val temp = ptk.eval(targetTree, getTrainTree(index))
          if(norm)
    ...
    if(interactive) {
      ...
      while(continue) {
          ...
          print("Target is predicted to be ")
		  try{ print(if(predict(target, convertMap) > 0) "\n<FAKE>\n" else "\n<AUTHENTIC>\n") } catch { case e:Exception => print("\n<Error: Unpredictable>\n") }
		  println
    		}
      }
    } else {
      println("Make prediction to all the test data.")
    }
    println
    ...
    mapStr match {
      case Some(x) =>
        // タグ・数値変換データファイルを上書き(UTF-8)
          ...
          val pWriter = new PrintWriter(new BufferedWriter(
              new OutputStreamWriter(new FileOutputStream(convertFile),"UTF-8")))
          pWriter.write(x)
          pWriter.close()
